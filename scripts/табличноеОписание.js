function изменитьТаблицу(event){
  ...
  if(event.keyCode==13 && this.className=="последняяСтрока"){
	создатьСтроку();
  } else if(event.keyCode==13 && this.className!="последняяСтрока" && номерБазыКопии==""){
	изменитьТело();
  } else if(event.keyCode==13 && this.className!="последняяСтрока" && номерБазыКопии!=""){
...
function создатьСтроку(){
    ...
    трансформаторСтруктуры.setAttribute("оськп", document.getElementsByName("осьXтабл")[N-1].value+","+document.getElementsByName("осьYтабл")[N-1].value+","+document.getElementsByName("осьZтабл")[N-1].value);
	...
	новаяСтрока.addEventListener('keydown',изменитьТаблицу);
}
...
function перестроитьТаблицу(){
  ...
  for(var i=0; i<=N+1; i++){
   var строка;
   if(i!=N+1 /*|| N==0*/){
	 строка = document.createElement("tr");
	...
	for(var j=0; j<=5; j++){
	  ...
	  // последняя строка
     строка = document.getElementById("строка_"+N).cloneNode(true);
    таблица.appendChild(строка);
    строка.className = "последняяСтрока";
    строка.id = "строка_"+(N+1);
	строка.children[0].firstChild.textContent = N+1;
	
  }
  строка.addEventListener('keydown',изменитьТаблицу);
 // document.getElementById("ТПМС").addEventListener('keydown',изменитьТаблицу);
  }
}
...
function удалитьПоследнююСтрокуТабл(){
  document.getElementById("тело_"+N).parentNode.removeChild(document.getElementById("тело_"+N));
   /* var последняяСтрока = document.getElementById("ТПМС").getElementsByTagName('tbody')[0].getElementsByTagName("tr")[N];
  последняяСтрока.parentNode.removeChild(последняяСтрока);*/
  перестроитьТаблицу();
